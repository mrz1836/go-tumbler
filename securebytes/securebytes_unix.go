...
func mlock(b []byte) error {
	if len(b) == 0 {
		return nil // mlock of a zero-length region is a no-op; some kernels reject it.
	}
	return unix.Mlock(b)
}
...
// before the first mlock, via raiseOnce in New / NewZero.
//
...
// make a large enrollment fail with ENOMEM. The raise only ever moves the
// soft limit up to the already-permitted hard limit, so it cannot grant
// more locked memory than the OS already allows.
//
